document.addEventListener('DOMContentLoaded', function() {
    ...
    paymentForm.addEventListener('submit', function(event) {
        ...
        if (paymentMethod === 'wallet') {
            if (amount <= walletBalance) {
                ...
                addTransactionToHistory(amount, paymentMethod);
                paymentMessage.textContent = 'Payment successful!';
                paymentMessage.classList.add('text-success');
                paymentMessage.classList.remove('text-danger');
            } else {
                paymentMessage.textContent = 'Insufficient wallet balance!';
    ...
    function addTransactionToHistory(amount, method) {
        const li = document.createElement('li');
        const date = new Date().toISOString().split('T')[0];
        li.textContent = `Amount: $${amount.toFixed(2)} | Method: ${method} | Date: ${date}`;
