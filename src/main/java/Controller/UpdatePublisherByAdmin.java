...
public class UpdatePublisherByAdmin extends HttpServlet {
   private static Set<String> generatedNames = new HashSet<>();
    ...
    throws ServletException, IOException {
        ...
        String Description = "This account is violated our community rule";
        
        Publishers publish = PublisherDAO.getPublisherByPublisherId(UserId);
        if (publish !=null){
                PublisherDAO.updateDefaultPublisher(newN,AvartaLink, UserId, Description);
                String Email = publish.getGmail();
            ...
            //request.setAttribute("gamer", gamer);
             response.sendRedirect("profileServlet?userid="+ publish.getId());     
    } 
